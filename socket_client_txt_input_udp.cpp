// g++ socket_client_txt_input_udp.cpp -std=c++17 -o client && ./client CME txt
#include <iostream>
#include <string>
#include <cstring>
#include <map>
...
void client_program(int port, const std::string &exchange, const std::string &outputType)
{
    ...
    if (recvfrom(client_socket, buffer, sizeof(buffer), 0, reinterpret_cast<struct sockaddr *>(&server_addr), &server_addr_length) == -1)
    {
